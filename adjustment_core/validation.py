...
def haversine_distance(coords1: np.ndarray, coords2: np.ndarray) -> np.ndarray:
    """Compute Haversine distance (meters) between two coordinate sets."""
    R = 6371000
    lat1 = coords1[:, 0]
    lon1 = coords1[:, 1]
    lat2 = coords2[:, 0]
...
def check_pair_error(img_i: RSImage, img_j: RSImage) -> np.ndarray:
    """Compute tie-point error for one image pair (i, j)."""
...
def calculate_error_report(images: List[RSImage],
    ...
    for (i, j) in overlapping_pairs:
        ...
        if len(distances) > 0:
            ...
            if verbose and is_rank_0:
                print(
                    f"Pair ({i}, {j}) | Points: {len(distances)} | Mean Error: {distances.mean():.4f} m | Median Error: {np.median(distances):.4f} m")
